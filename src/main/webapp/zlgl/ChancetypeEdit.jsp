...
<script>
    $(function () {
        ...
        $.ajax({
            type: 'post',
            url: '../FindChancetypeDateBind.action',
            dataType: 'json',
            data: {
                CtId: $('#ids').val()
            }, success: function (data) {
                $('#CtName').val(data.c.ctName);
                $('#Ptid').val(data.c.ptId);
                $('#CtDescribe').val(data.c.ctDescribe);
                $('#Compid').val(data.c.compid);
                $('#CtUnit').val(data.c.ctUnit);
                $('#CtUnitprice').val(data.c.ctUnitprice);
                $('#CtId').val(data.c.ctId);
            }
        })
    })

    function ChancetypeEdit() {
        $.ajax({
            type: "POST",
            url: "../FindChancetypeEdit.action",
            dataType: "text",
            data: {
                CtName: $('#CtName').val(),
                PtId: $('#Ptid').val(),
...
<div id="distpicker" data-toggle="distpicker">
    ...
    <div class="layui-form-item">
        <div class="layui-input-block">
            <button class="layui-btn" lay-submit lay-filter="formDemo" onclick="ChancetypeEdit();">立即提交</button>
