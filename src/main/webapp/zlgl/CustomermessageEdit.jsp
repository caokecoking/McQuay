...
<script>
    $(function () {
        ...
        $.ajax({
            type: 'post',
            url: '../FindCustomermessageDateBind.action',
            dataType: 'json',
            data: {
                CustId: $('#ids').val()
            },
            success: function (data) {
                $("#CustId").val(data.c.custId);
                $('#CustAddress').val(data.c.custAddress);
                $('#CustName').val(data.c.custName);
                $('#CustProject').val(data.c.custProject);
                $('#CustType').val(data.c.custType);
                $('#cd').val(data.c.compid);
                $("#distpicker").distpicker('destroy');
                $('#distpicker').distpicker({
                    province: data.c.prov,
                    city: data.c.dist,
                    district: data.c.coun
                });
            }
        })
    ...
    function CustomermessageEdit() {
        $.ajax({
            type: "POST",
            url: "../FindCustomermessageEdit.action",
            dataType: "text",
            data: {
                CustId: $('#CustId').val(),
...
<div class="layui-form">
    ...
    <div class="layui-form-item">
        <div class="layui-input-block">
            <button class="layui-btn" lay-submit lay-filter="formDemo" onclick="CustomermessageEdit();">立即提交</button>
